            ...
            <div data-reveal="stagger" className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              {services.map((service) => (
                <div key={service.title} className="rounded-xl border border-white/10 bg-white/5 p-5">
                  <h3 className="mb-4 font-serif text-xl text-gold">{service.title}</h3>
                  <ul className="space-y-2 text-sm text-slate-300">{service.items.map((item) => <li key={item}>• {item}</li>)}</ul>
                </div>
              ))}
            </div>
          </div>
